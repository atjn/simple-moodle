...
const setView = () => {
	if(coursesView.dataset.display !== "list"){
		const listSelector = overview.querySelector(`#displaydropdown + ul a[data-value="list"]`);
		listSelector?.click();
...
const setPaging = () => {
	if(coursesView.dataset.paging !== "24"){
		const pagingSelector = overview.querySelector(`.dropdown-menu > a.dropdown-item[data-limit="24"]`);
...
const setGrouping = () => {
	if(coursesView.dataset.grouping !== "favourites"){
		const favoritesSelector = overview.querySelector(`#groupingdropdown + ul a[data-value="favourites"]`);
	...
	for(const courseItem of document.querySelectorAll(".rui-course-listitem")){
		...
		if(iconSpan && icon && iconSpan.dataset.simpleMoodleObserving !== "true"){
			icon.addEventListener("click", event => {
				...
				const action = iconSpan.classList.contains("hidden") ? "add" : "remove";
				const control = courseItem.querySelector(`.dropdown-menu .dropdown-item[data-action="${action}-favourite"]`);
				control?.click();
			});
